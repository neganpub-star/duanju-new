<template>
	<view class="lime-dialer" :style="getStyle">
		<view class="lime-dialer__inner" :style="getDialStyle">
			...
			<view class="lime-dialer__inner-wrap" :style="styleOpt.borderColor && (' border-top: 16rpx solid ' + styleOpt.borderColor)">
		...
		<!--  :style="pointerStyle" -->
		<view class="lime-dialer__pointer" @click="run(0)" >
			...
			<image
				...
				@tap="$emit('click')"
			/>
			<view style="position: absolute;z-index: 100; left: 38%;top: 35%;display: flex;flex-direction: column;align-items: center;">
				<view style="font-size: 40rpx;font-weight: 700;color: #ff633c;">抽奖</view>
				<view style="font-size: 22rpx;font-weight: 700;color: #ff633c;">今日剩余</view>
				<view style="font-size: 22rpx;font-weight: 700;color: #ff633c;">4次</view>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	props: {
		...
		styleOpt: {
			...
			default: () => ({
				...
				// $primary-1 $primary-2 
				prizeBgColors: ['radial-gradient(#fff3d2, #fffcf5)', '	(#ffa269, #ff7769)'],
				// 每一块扇形的外边框颜色,默认值,可通过父组件来改变
				// primary-4
				// borderColor: 'radial-gradient(#ffa269, #ff7769)',
			})
		},
		customStyle: {
			type: String,
	...
	computed: {
		getRotateAngle() {
			return index => {
				return {
					...
					background: `${this.styleOpt.prizeBgColors[index % this.styleOpt.prizeBgColors.length]}`,
		...
		getStyle() {
			...
			return `width: 676rpx; height: 676rpx; ${customStyle}`;
		},
		getDialStyle() {
			return `
