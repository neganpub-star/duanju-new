				   ...
				   let that = this
					...
					tt.login({
						...
						success(res){
								...
								that.$request('douyin.douyinlogin', { code: res.code }).then(res => {
									if(res.code === 1) {
										if(res.data.token) {
											that.$u.toast("登录成功")
										...
										if(res.data.user_oauth_id) {
											that.login.auth = res.data.user_oauth_id
											if(that.config.system.mobile_switch == 1) {
												that.login.bind = true
											} else {
												that.bindAuthMobile(false)
											}
										}
									}
								})
