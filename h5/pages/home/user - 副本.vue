...
<template>
	<view class="page_content">
		...
		<view class="main_content" style="background: rgba(235, 236, 237, 1);" v-if="isStatus == 0">
			<view class="userinfo_box" style="flex-direction: column;margin-bottom: 50rpx;" v-if="userInfoStore"
			...
			<view class="userinfo_box" style="margin-bottom: 50rpx;flex-direction: column;" v-else
			...
			<view class="menu_box" v-if="userInfoStore.mgg == 1 || !mggStatus"
				style="	padding:0 20rpx;background: #fff;">
				...
				<view style="display: flex; flex-wrap: wrap;padding:0 20rpx;">
					<view v-if="item.id!=8" class="item" style="flex: 0 0 24%;  margin-left: calc(4% / 4);  "
						v-for="(item, index) in menuListTwo" :key="item.id"
						@click="menuItemClick(item.rid, item.text, item.path)">
						<view class="left" v-if="userInfoStore.mgg == 1 "
							style="flex-direction: column;justify-content: center;align-items: center;">
							<view class="icon" v-if="item.id!= 8"
								style="width: 36rpx;height: 38rpx;margin-bottom: 16rpx;">
								<image class="image" :src="item.img" :style="{ width: item.width }" mode="widthFix">
								</image>
							</view>
							<view v-if="item.id!= 8" class="text" style="margin-left: 0;color: rgba(70, 74, 87, 1);">
								{{ item.text }}</view>
						</view>
			...
			<view class="menu_box" v-else style="	padding:0 20rpx;background: #fff;">
				<view style="font-size: font-size: 32rpx;color: rgba(55, 57, 72, 1);padding: 20rpx 0 10rpx 20rpx;">
					全部功能
				</view>
				<view style="display: flex; flex-wrap: wrap;padding:0 20rpx;">
					<view class="item" style="flex: 0 0 24%;  margin-left: calc(4% / 4);  "
						v-for="(item, index) in menuListTwo" :key="item.id"
						@click="menuItemClick(item.rid, item.text, item.path)">
						<view class="left" v-if="userInfoStore.mgg == 1 "
							style="flex-direction: column;justify-content: center;align-items: center;">
		...
		<view class="main_content" v-if="isStatus == 1">
			...
			<!-- || !mggStatus -->
			<view class="menu_box" v-if="userInfoStore.mgg == 1 || !mggStatus">
				<view v-if="item.id!=8" class="item" v-for="(item, index) in menuList" :key="item.id"
					@click="menuItemClick(item.rid, item.text, item.path)">
					<view class="left">
						<view class="icon">
		...
		.main_content {
			...
			.vip_card {
				...
				.vip_box {
					...
					background: linear-gradient(141.96deg, hotpink 0%, pink 100%);
